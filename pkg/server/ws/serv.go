...
func (sl *ServeLoop)runThread() {
	...
	t,ok := sl.waitLoopP.Get().(*WsLoopCtx)
	if !ok {return}

	slm_ptr,p_ok := t.Args.(*ServeLoopCtxValue)
	if !p_ok {panic("ServeLoop structure assertion error")}
...
func (sl *ServeLoop)closeLoop() {
	t,ok := sl.closedLoopP.Get().(*WsLoopCtx)
	if !ok {return}

	t.Obj.Close()
	t.Conn.Close()
}
func (sl *ServeLoop)errAndClose(t *WsLoopCtx,err error) {
